...
// src/content-type.ts
var isTextResponse = (contentType) => {
  const lowered = contentType.toLowerCase();
  return lowered.startsWith("text/") || lowered.includes("json") || lowered.includes("javascript") || lowered.includes("xml") || lowered.includes("svg") || lowered.includes("html");
};
...
var stripLeadingSlash = (value) => value.replace(/^\/+/, "");
var ensureLeadingSlash = (value) => value.startsWith("/") ? value : `/${value}`;
var sanitizePosixPath = (value) => {
...
var bodyToTextOrBase64 = (bytes, mimeType) => {
  if (isUtf8Text(bytes, mimeType)) {
    const text3 = decodeUtf8(bytes) ?? "";
    return { encoding: "text", text: text3 };
  }
...
var networkIdle = (ms, checkInterval = 100) => ({
  async wait(ctx) {
    while (true) {
...
var normalizeCompletion = (completion) => {
  if (!completion) return [];
  return Array.isArray(completion) ? completion : [completion];
};

// ../uni-fs/dist/index.js
var OPFS_ERROR_MESSAGE = "OPFS is not available in this environment. Use a browser or service worker that supports navigator.storage.getDirectory().";
...
async function getOpfsDirectory(root2, segments, options) {
  let current = root2;
...
async function dataToOpfsWritable(data2) {
...
async function dataToNodeWritable(data2) {
  if (typeof data2 === "string") {
    return data2;
  }
  if (data2 instanceof Uint8Array) {
  ...
  if (typeof Blob !== "undefined" && data2 instanceof Blob) {
    return new Uint8Array(await data2.arrayBuffer());
  }
  throw new Error("Unsupported data type.");
}
function getNodeCwd() {
...
async function readBinary(filename, extension) {
  ...
  if (isOpfsAvailable()) {
    ...
    const directory = await getOpfsDirectory(root2, directories, {
      create: false
    });
...
async function write(filename, extension, data2) {
  const fileName = buildFileName(filename, extension);
  if (isOpfsAvailable()) {
    const root2 = await getOpfsRoot();
    const { directories, basename } = splitPath(fileName);
    ...
    const writableData = await dataToOpfsWritable(data2);
  ...
  if (isNodeEnvironment()) {
    ...
    const writableData = await dataToNodeWritable(data2);
    await writeFile(outputPath, writableData);
    return;
  }
  throw new Error(OPFS_ERROR_MESSAGE);
}
async function remove(filename, extension) {
  const fileName = buildFileName(filename, extension);
  if (isOpfsAvailable()) {
    const root2 = await getOpfsRoot();
    const { directories, basename } = splitPath(fileName);
...
var DEFAULT_THRESHOLD = 256 * 1024;
var DEFAULT_BASE_DIR = ".pagepocket_store";
var nowId = () => `${Date.now()}_${Math.random().toString(16).slice(2)}`;
var streamFromBytes = (data2) => new ReadableStream({
  start(controller) {
...
var HybridContentStore = class {
  constructor(options) {
    this.name = "hybrid";
    this.storedIds = /* @__PURE__ */ new Set();
    this.thresholdBytes = options?.thresholdBytes ?? DEFAULT_THRESHOLD;
    this.baseDir = options?.baseDir ?? DEFAULT_BASE_DIR;
  }
  async put(body, meta) {
    const data2 = await toUint8Array(body);
    const size = data2.byteLength;
    if (size <= this.thresholdBytes) {
      return { kind: "memory", data: data2 };
    }
    const id = `${hashString(meta.url)}_${nowId()}`;
    await write(`${this.baseDir}/${id}`, "bin", data2);
  ...
  async open(ref) {
    ...
    const data2 = await readBinary(`${this.baseDir}/${ref.id}`, "bin");
    return streamFromBytes(data2);
  }
  async dispose() {
...
var responseMimeType = (response) => response.mimeType || getHeaderValue(response.headers || {}, "content-type");
var NetworkStore = class {
  constructor(options) {
    this.requests = /* @__PURE__ */ new Map();
    this.storedResources = [];
    this.apiEntries = [];
    this.apiRecordIds = /* @__PURE__ */ new Set();
    this.warnings = [];
    this.totalBytes = 0;
  ...
  async handleEvent(event) {
    ...
    if (this.limits.maxSingleResourceBytes && bodyBytes.byteLength > this.limits.maxSingleResourceBytes) {
    ...
    if (this.limits.maxTotalBytes && this.totalBytes + bodyBytes.byteLength > this.limits.maxTotalBytes) {
...
var sameDomain = (left, right) => left.hostname === right.hostname;
var createDefaultPathResolver = () => {
  const resolvedByUrl = /* @__PURE__ */ new Map();
...
var resolveCrossOrigin = (url, entryUrl) => {
  try {
...
var withPrefixPathResolver = (resolver, prefix) => {
...
// src/resource-filter.ts
var DEFAULT_ALLOWED = /* @__PURE__ */ new Set(["document", "stylesheet", "script", "image", "font", "media"]);
var isSkippableUrl = (url) => url.startsWith("data:") || url.startsWith("blob:") || url.startsWith("mailto:") || url.startsWith("tel:") || url.startsWith("javascript:");
var createDefaultResourceFilter = () => ({
  shouldSave(req, res) {
...
var IMPORT_PATTERN = /@import\s+(?:url\()?['"]?([^'")]+)['"]?\)?/g;
var shouldSkipValue = (value) => {
  const trimmed = value.trim();
  return !trimmed || trimmed.startsWith("data:") || trimmed.startsWith("blob:") || trimmed.startsWith("mailto:") || trimmed.startsWith("tel:") || trimmed.startsWith("javascript:") || trimmed.startsWith("#");
};
var rewriteCssText = async (input) => {
  const { cssText, cssUrl, resolveUrl } = input;
  let updated = "";
  let lastIndex = 0;
  for (const match of cssText.matchAll(URL_PATTERN)) {
    const index2 = match.index ?? 0;
    updated += cssText.slice(lastIndex, index2);
    const quote = match[1] || "";
  ...
  for (const match of updated.matchAll(IMPORT_PATTERN)) {
    const index2 = match.index ?? 0;
    final += updated.slice(lastIndex, index2);
...
// src/hackers/preload-fetch.ts
var preloadFetchRecorder = {
...
// src/hackers/preload-xhr.ts
var preloadXhrRecorder = {
...
// src/hackers/replay-beacon.ts
var replayBeaconStub = {
...
// src/hackers/replay-dom-rewrite.ts
var replayDomRewriter = {
...
// src/hackers/replay-eventsource.ts
var replayEventSourceStub = {
...
// src/hackers/replay-fetch.ts
var replayFetchResponder = {
...
// src/hackers/replay-svg-image.ts
var replaySvgImageRewriter = {
...
// src/hackers/replay-websocket.ts
var replayWebSocketStub = {
...
// src/hackers/replay-xhr.ts
var replayXhrResponder = {
...
// src/hackers/index.ts
var preloadHackers = [preloadFetchRecorder, preloadXhrRecorder];
var replayHackers = [
...
// src/preload.ts
var buildPreloadScript = () => {
  const context = { stage: "preload" };
  const hackerScripts = preloadHackers.map((hacker) => `  // hacker:${hacker.id}
${hacker.build(context)}`).join("\n");
  return `
(function () {
...
function matchAPI(options) {
  ...
  const stripHash = (value) => {
    ...
    return index2 === -1 ? value : value.slice(0, index2);
  };
...
var hackHtml = (input) => {
  ...
  if (head.length) {
    ...
    root2.prepend(replayScript);
    root2.prepend(preloadScript);
  }
  if (input.faviconDataUrl) {
    ...
    if (existingIcon.length) {
      ...
      root2.append(`<link rel="icon" href="${input.faviconDataUrl}" />`);
    }
  }
};

// src/rewrite-links.ts
var shouldSkipValue2 = (value) => {
  const trimmed = value.trim();
  return !trimmed || trimmed.startsWith("data:") || trimmed.startsWith("blob:") || trimmed.startsWith("mailto:") || trimmed.startsWith("tel:") || trimmed.startsWith("javascript:") || trimmed.startsWith("#");
};
var resolveUrlValue = (value, baseUrl, resolve) => {
  if (shouldSkipValue2(value)) {
    return null;
  }
  try {
...
var rewriteSrcsetValue = (value, baseUrl, resolve) => {
...
var rewriteMetaRefresh = (content, baseUrl, resolve) => {
  ...
  const next2 = `url=${resolved}`;
  const nextParts = parts.slice();
  nextParts[urlPartIndex] = next2;
  return nextParts.join(";");
};
var rewriteJsText = async (source, resolve, baseUrl) => {
  const replaceSpecifier = async (specifier) => {
    const trimmed = specifier.trim();
    if (shouldSkipValue2(trimmed)) {
      return specifier;
    }
  ...
  for (const match of source.matchAll(importFromPattern)) {
    const index2 = match.index ?? 0;
    replaced += source.slice(lastIndex, index2);
    const prefix = match[1] || "";
    const quote = match[2] || "";
    const specifier = match[3] || "";
    const next2 = await replaceSpecifier(specifier);
    replaced += `${prefix}${quote}${next2}${quote}`;
    lastIndex = index2 + match[0].length;
  }
  ...
  for (const match of replaced.matchAll(importSideEffectPattern)) {
    const index2 = match.index ?? 0;
    final += replaced.slice(lastIndex, index2);
    const prefix = match[1] || "";
    const quote = match[2] || "";
    const specifier = match[3] || "";
    const next2 = await replaceSpecifier(specifier);
    final += `${prefix}${quote}${next2}${quote}`;
    lastIndex = index2 + match[0].length;
  }
  ...
  for (const match of final.matchAll(dynamicImportPattern)) {
    const index2 = match.index ?? 0;
    dynamicFinal += final.slice(lastIndex, index2);
    const prefix = match[1] || "";
    const quote = match[2] || "";
    const specifier = match[3] || "";
    const suffix = match[4] || "";
    const next2 = await replaceSpecifier(specifier);
    dynamicFinal += `${prefix}${quote}${next2}${quote}${suffix}`;
    lastIndex = index2 + match[0].length;
  }
  dynamicFinal += final.slice(lastIndex);
  return dynamicFinal;
};
var rewriteEntryHtml = async (input) => {
  const $2 = load(input.html);
  const baseUrl = input.entryUrl;
  const resolve = input.resolve;
  const shouldRewriteLinks = input.rewriteLinks !== false;
  const rewriteAttr = (selector, attr2) => {
    $2(selector).each((_, element) => {
      const value = $2(element).attr(attr2);
      if (!value) return;
      const resolved = resolveUrlValue(value, baseUrl, resolve);
      if (resolved) {
        $2(element).attr(attr2, resolved);
      }
    });
  };
  const rewriteDataAttrs = (selector, attr2) => rewriteAttr(selector, attr2);
  if (shouldRewriteLinks) {
    ...
    $2("img[srcset], source[srcset]").each((_, element) => {
      const value = $2(element).attr("srcset");
      if (!value) return;
      const rewritten = rewriteSrcsetValue(value, baseUrl, resolve);
      $2(element).attr("srcset", rewritten);
    });
    $2("meta[http-equiv]").each((_, element) => {
      const httpEquiv = ($2(element).attr("http-equiv") || "").toLowerCase();
      if (httpEquiv !== "refresh") return;
      const content = $2(element).attr("content");
      if (!content) return;
      const rewritten = rewriteMetaRefresh(content, baseUrl, resolve);
      $2(element).attr("content", rewritten);
    });
  }
  if (shouldRewriteLinks) {
    const inlineStyles = $2("style").toArray();
    for (const element of inlineStyles) {
      const cssText = $2(element).html();
      if (!cssText) continue;
      ...
      if (rewritten !== cssText) {
        $2(element).html(rewritten);
      }
    }
    const inlineStyleElements = $2("[style]").toArray();
    for (const element of inlineStyleElements) {
      const styleText = $2(element).attr("style");
      ...
      if (rewritten !== styleText) {
        $2(element).attr("style", rewritten);
      }
    }
  }
  if (shouldRewriteLinks) {
    const moduleScripts = $2('script[type="module"]').toArray();
    for (const element of moduleScripts) {
      const src = $2(element).attr("src");
      if (src) continue;
      const original = $2(element).html();
  ...
  const title = $2("title").first().text() || void 0;
  return { html: $2.html(), title };
};

// src/writers.ts
var normalizePath = (value) => value.replace(/\\/g, "/");
var joinPath = (base, relative) => {
...
var streamToUint8Array = async (stream) => {
...
var writeToFS = async (snapshot, outDir, options) => {
  let filesWritten = 0;
  let totalBytes = 0;
  for (const file of snapshot.files) {
    ...
    const data2 = await streamToUint8Array(stream);
    await write(filename, extension, data2);
    filesWritten += 1;
    totalBytes += data2.byteLength;
  }
...
var crc32 = (data2) => {
  ...
  for (let i = 0; i < data2.length; i += 1) {
    crc = crc >>> 8 ^ crc32Table[(crc ^ data2[i]) & 255];
  }
  return (crc ^ -1) >>> 0;
};
var writeUint16 = (value) => {
  const buffer = new Uint8Array(2);
  const view = new DataView(buffer.buffer);
  view.setUint16(0, value, true);
  return buffer;
};
var writeUint32 = (value) => {
...
var toZip = async (snapshot, options) => {
  const localChunks = [];
  const centralChunks = [];
  let offset = 0;
  for (const file of snapshot.files) {
    ...
    const localHeader = concatBytes([
      ...
      writeUint16(0),
      writeUint32(crc),
      writeUint32(data2.byteLength),
      writeUint32(data2.byteLength),
      writeUint16(nameBytes.byteLength),
      writeUint16(0),
      nameBytes
    ]);
    localChunks.push(localHeader, data2);
    ...
    offset += localHeader.byteLength + data2.byteLength;
  }
  ...
  const output = options?.asBlob && typeof Blob !== "undefined" ? new Blob([zipBytes], { type: "application/zip" }) : zipBytes;
  if (options?.clearCache ?? true) {
    await snapshot.content.dispose?.();
  }
  return output;
...
// src/snapshot.ts
var createPageSnapshot = (data2) => {
  return {
    ...data2,
    toDirectory: (outDir, options) => writeToFS(data2, outDir, options),
    toZip: (options) => toZip(data2, options)
  };
};

// src/snapshot-builder.ts
var streamToUint8Array2 = async (stream) => {
  const reader = stream.getReader();
  const chunks = [];
  let total = 0;
  while (true) {
...
var groupResources = (input) => {
  ...
  if (!hasFrameId) {
    ...
    for (const resource of input.resources) {
      ...
      primaryGroup2.resources.push(resource);
      if (resource.request.resourceType === "document") {
        primaryGroup2.docResource = resource;
      }
    }
    for (const apiEntry of input.apiEntries) {
      primaryGroup2.apiEntries.push(apiEntry);
    }
    return [primaryGroup2];
  }
  ...
  const primaryGroup = primaryDoc ? groups.get(primaryDoc.request.frameId ?? primaryDoc.request.requestId) ?? null : null;
  const groupByUrl = /* @__PURE__ */ new Map();
  ...
  for (const resource of input.resources) {
    ...
    const byFrame = frameId ? groups.get(frameId) : void 0;
    const byInitiator = resource.request.initiator?.url ? groupByUrl.get(resource.request.initiator.url) : void 0;
  ...
  for (const entry of input.apiEntries) {
    ...
    const byFrame = frameId ? groups.get(frameId) : void 0;
    const byInitiator = entry.request.initiator?.url ? groupByUrl.get(entry.request.initiator.url) : void 0;
    const target = byFrame ?? byInitiator ?? primaryGroup ?? Array.from(groups.values())[0];
    if (target) {
...
var buildApiSnapshot = (url, createdAt, entries) => ({
  version: "1.0",
  url,
  createdAt,
  records: entries.map((entry) => entry.record)
});
var buildSnapshot = async (input) => {
  ...
  for (const group of groups) {
    ...
    for (const resource of group.resources) {
      if (resource.request.resourceType === "document") {
        const path2 = urlToPath.get(resource.request.url) ?? "/index.html";
        const stream = await input.contentStore.open(resource.contentRef);
        const bytes = await streamToUint8Array2(stream);
        const decoded = decodeUtf8(bytes) ?? "";
        let html3 = decoded;
        ...
        html3 = rewritten.html;
        if (!title) {
          title = rewritten.title;
        }
        const encoded = new TextEncoder().encode(html3);
        const contentRef2 = await input.contentStore.put(
      ...
      const path = urlToPath.get(resource.request.url) ?? resolver.resolve({
        url: resource.request.url,
        resourceType: resource.request.resourceType,
        mimeType: resource.mimeType,
        suggestedFilename: void 0,
...
// src/pagepocket.ts
var PagePocket = class _PagePocket {
  constructor(target, options) {
    this.target = target;
    this.options = options ?? {};
  }
  static fromURL(url, options) {
    return new _PagePocket({ kind: "url", url }, options);
  }
  static fromTarget(target, options) {
    return new _PagePocket(target, options);
  }
  async capture(options) {
    ...
    const filter4 = options?.filter ?? createDefaultResourceFilter();
    ...
    const completion = completionStrategies.length > 0 ? completionStrategies : [networkIdle(1e3), timeout(5e3)];
